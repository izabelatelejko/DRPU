...
from DRPU.dataset import (
    get_synthetic_positive,
    get_synthetic_test,
    get_synthetic_unlabeled,
    Tensor_to_1darray,
)
...
def uPU(
    ...
    (
        ...
        _,
        _,
    ) = load_trainset(dataset_name, train_size, val_size, batch_size, true_train_prior)
    ...
    for true_prior in true_test_priors:
        testloader, testset = load_testset(
            dataset_name, test_size, batch_size, true_prior
        )
    ...
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=lr, weight_decay=0.1, betas=(0.5, 0.999)
    )
    ...
    save_test_history(
        os.path.join(res_dir, "train", "history_{}".format(id)), test_results
    )
    ...
    for i, (true_prior, result) in enumerate(zip(true_test_priors, test_results)):
        ...
        append_test_results(
            getdirs(os.path.join(res_dir, "test-{}".format(i))),
            acc,
            auc,
            boundary=boundary,
        )
...
def DRPU(
    ...
    trainloader_P, trainloader_U, valloader_P, valloader_U, _, trainset_U, _, _ = (
        load_trainset(dataset_name, train_size, val_size, batch_size, true_train_prior)
    )
