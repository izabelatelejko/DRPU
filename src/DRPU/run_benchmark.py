...
from DRPU.dataset import (
    get_image_positive,
    get_image_test,
    get_image_unlabeled,
    Tensor_to_1darray,
)
from DRPU.model import choose_model
from DRPU.metric import *
from DRPU.save import *
...
def load_trainset(dataset_name, train_size, val_size, batch_size, data_dir="dataset"):
    ...
    trainset_P = get_image_positive(
        dataset_name, trainsize_P, train_indices, root=data_dir
    )
    ...
    trainloader_P = torch.utils.data.DataLoader(
        trainset_P,
        batch_size=len(trainset_P) // batch_num,
        shuffle=True,
        drop_last=True,
        num_workers=2,
    )
    trainloader_U = torch.utils.data.DataLoader(
        trainset_U, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=2
    )
    valloader_P = torch.utils.data.DataLoader(
        valset_P,
        batch_size=len(valset_P) // batch_num,
        shuffle=False,
        drop_last=False,
        num_workers=1,
    )
    valloader_U = torch.utils.data.DataLoader(
        valset_U, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=1
    )
    return (
        trainloader_P,
        trainloader_U,
...
def load_testset(dataset_name, batch_size, prior=None, data_dir="dataset"):
    ...
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=1
    )
...
def uPU(
    ...
    device = (
        torch.device(device_num)
        if device_num >= 0 and torch.cuda.is_available()
        else "cpu"
    )
    ...
    (
        ...
        _,
        _,
    ) = load_trainset(dataset_name, train_size, val_size, batch_size, data_dir)
    ...
    testloaders = [
        load_testset(dataset_name, batch_size, true_prior, data_dir)[0]
        for true_prior in true_test_priors
    ]

    model = choose_model(dataset_name)().to(device)
    optimizer = torch.optim.Adam(
        params=model.parameters(), lr=lr, weight_decay=0.005, betas=(0.9, 0.999)
    )
    ...
    save_train_history(
        getdirs(os.path.join(res_dir, "train", "history_{}".format(id))),
        model,
        train_result,
    )
    output_train_results(
        os.path.join(res_dir, "log_{}.txt".format(id)), train_result, train_prior
    )
    save_test_history(
        getdirs(os.path.join(res_dir, "train", "history_{}".format(id))), test_results
    )
    ...
    for i, (true_prior, result) in enumerate(zip(true_test_priors, test_results)):
        ...
        auc = result.get("auc")
        output_test_results(
            os.path.join(res_dir, "log_{}.txt".format(id)), i, true_prior, acc, auc
        )
        append_test_results(
            getdirs(os.path.join(res_dir, "test-{}".format(i))), acc, auc
        )

    output_config(
        os.path.join(res_dir, "log_{}.txt".format(id)),
        train_size,
        val_size,
        max_epochs,
        batch_size,
...
def PUa(
    ...
    for i, true_prior in enumerate(true_test_priors):
        testloader, testset = load_testset(
            dataset_name, batch_size, true_prior, data_dir
        )
        trans = Tensor_to_1darray()
        ...
        criterion = AsymmetricNonNegativeRiskEstimator(
            train_prior, test_prior, choose_loss(loss_name)
        )
        ...
        save_train_history(
            getdirs(
                os.path.join(res_dir, "train-{}".format(i), "history_{}".format(id))
            ),
            model,
            train_result,
        )
        ...
        append_test_results(
            getdirs(os.path.join(res_dir, "test-{}".format(i))), acc, auc, test_prior
        )
...
def DRPU(
    ...
    trainloader_P, trainloader_U, valloader_P, valloader_U, _, _, _, _ = load_trainset(
        dataset_name, train_size, val_size, batch_size, data_dir
    )
    ...
    if true_train_prior is None:
        ...
        given_thresholds = [
            ...
            * (1 - test_prior)
            / (
                train_prior
                * ((1 - train_prior) * test_prior + train_prior * (1 - test_prior))
                + EPS
            )
    ...
    for i, (true_prior, result) in enumerate(zip(true_test_priors, test_results)):
        ...
        append_test_results(
            getdirs(os.path.join(res_dir, "test-{}".format(i))), acc, auc, prior, thresh
        )
